...
export default function RegisterPage() {
	...
	return (
		...
		<div className="relative min-h-screen flex items-center justify-center bg-zinc-950 px-4 py-12 overflow-hidden select-none">
			{/* Background Ambient Gradient Blobs (Matching the login screen's atmosphere) */}
			...
			<div className="relative w-full max-w-md bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/60 rounded-3xl p-6 sm:p-10 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7)] z-10">
				...
				<div className="text-center mb-8">
					...
					<h1 className="text-white text-3xl font-extrabold tracking-tight">Create Account</h1>
					<p className="text-zinc-400 mt-2 text-sm font-medium">Join Spotify to start listening</p>
				</div>
				...
				<form onSubmit={handleSubmit} className="space-y-5">
					...
					<div className="space-y-2">
						...
						<div className="relative">
							<select
								...
								value={formData.role}
								onChange={handleChange}
								className="w-full bg-zinc-950/60 text-white px-4 py-3.5 rounded-2xl border border-zinc-800/80 outline-none focus:border-green-500/80 focus:ring-4 focus:ring-green-500/10 transition-all duration-300 font-medium appearance-none cursor-pointer"
							>
								<option value="user" className="bg-zinc-900 text-white">
									User
								</option>
								<option value="artist" className="bg-zinc-900 text-white">
									Artist
								</option>
							</select>
							{/* Custom Down Arrow Icon for sleek design */}
					...
					{/* Error Alerts */}
					{error && (
						<div className="flex items-center gap-2 text-red-400 text-sm bg-red-500/5 border border-red-500/10 px-4 py-3 rounded-xl">
							<svg
